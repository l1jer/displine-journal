...
每次改动后启动项目，ESlint都会检测你的代码，然后在浏览器的控制台疯狂报错。根据控制台的报错信息，一条一条修改，这样的方式太低效。

高效的方式，可以直接让开发工具（webstorm or idea）识别ESlint，并且格式代码，过程是这样的：

`Setting ->Preferences -> Languages & Frameworks -> JavaScript -> Code Quality Tools -> Eslint`-> 勾选Enable。



### 4.如何关闭ESlint语法检测？

build --> webpack.base.conf.js，然后注释掉图片所指那段代码，就可以了。
